...
export function createParamsString(qp) {
  const paramArray = Object.keys(qp).reduce((prev, key) => {
    ...
    return [
      ...prev,
      `${encodeURIComponent(keyString)}=${encodeURIComponent(valueString)}`
    ];
  }, []);
...
export function parseParams(query) {
  return (
    (query &&
      query.split("&").reduce((prev, queryparam) => {
        if (queryparam[0] === "?") {
          queryparam = queryparam.substr(1);
        }
        const split = queryparam.match(/^.*?(?==)|[^=\n\r].*$/gm); // Split after first equal sign
